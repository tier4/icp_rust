...
pub fn estimate_transform(src: &Vec<Vector2>, dst: &Vec<Vector2>) -> Transform {
    ...
    for _ in 0..max_iter {
        ...
        transform = Transform::from_se2(&delta) * transform;
    }
    transform
}
...
///
/// $$T = \mathrm{Exp}(\mathbf{x}) = \exp([\mathbf{x}]\_{\times}),\\ \mathbf{x} \in \mathbb{R}^{3}, T \in \mathrm{SE}(2)$$
///
...
///
/// Utilizing the functions defined above, we express the parameter addition by the $\boxplus$ operator:
///
/// $$
/// \mathbf{x} \boxplus \mathbf{y} = \mathrm{Log}(\mathrm{Exp}(\mathbf{x}) \mathrm{Exp}(\mathbf{y}))
/// $$
///
/// In the ICP's problem setting, given two associated points $\mathbf{a}, \mathbf{b} \in \mathbb{R}^{2}$,
/// we aim to find the optimal tarnsformation parameter $\mathbf{x}^{*} \in \mathrm{R}^{2}$ that minimizes the difference between the transformed source point and
/// the destination point.
///
/// $$
/// \mathbf{x}^{*} = {\arg\min}_{\mathbf{x}} || \mathbf{r}(\mathbf{a}, \mathbf{b}; \mathbf{x}) ||^{2},\\;\\;
/// \mathbf{r}(\mathbf{a}, \mathbf{b};\mathbf{x}) = \mathrm{Exp}(\mathbf{x}) \dot{\mathbf{a}} - \dot{\mathbf{b}}
/// $$
///
/// The dot denotes the homogeneous operator, such that $\dot{\mathbf{a}} = \left[a_x, a_y, 1\right]$.
///
/// To formulate the Gauss-Newton update, we calculate the jacobian of the residual function with respect to the pose parameter.
///
/// \\[
/// \begin{aligned}
/// J
/// &= \lim_{\delta \to \mathbf{0}} \frac{\mathbf{r}(\mathbf{a}, \mathbf{b}; \mathbf{x} \boxplus \mathbf{\delta}) - \mathbf{r}(\mathbf{a}, \mathbf{b}; \mathbf{x})}{\mathbf{\delta}}  \\\\
/// &= \lim_{\delta \to \mathbf{0}} \frac{1}{\mathbf{\delta}} \left\[\mathrm{Exp}(\mathbf{x} \boxplus \mathbf{\delta}) - \mathrm{Exp}(\mathbf{x}) \right\] \dot{\mathbf{a}}  \\\\
/// &= \lim_{\delta \to \mathbf{0}} \frac{1}{\mathbf{\delta}} \left\[\mathrm{Exp}(\mathbf{x})\mathrm{Exp}(\mathbf{\delta}) - \mathrm{Exp}(\mathbf{x}) \right\] \dot{\mathbf{a}}
/// \end{aligned}
/// \\]
///
/// Since $||\mathbf{\delta}||$ is small, we can approximate its exponential by
/// the corresponding Lie algebra representation: $\mathrm{Exp}(\mathbf{\delta}) \approx I + [\mathbf{\delta}]_{\times}$.
///
/// Therefore,
///
/// $$
/// \begin{aligned}
/// J
/// &\approx \lim_{\delta \to \mathbf{0}} \frac{1}{\mathbf{\delta}} \left[\mathrm{Exp}(\mathbf{x})(I + [\mathbf{\delta}]\_{\times}) - \mathrm{Exp}(\mathbf{x}) \right] \dot{\mathbf{a}}  \\\\
/// &= \lim_{\delta \to \mathbf{0}} \frac{1}{\mathbf{\delta}} \left[\mathrm{Exp}(\mathbf{x}) + \mathrm{Exp}(\mathbf{x}) {[\mathbf{\delta}]}\_{\times} - \mathrm{Exp}(\mathbf{x}) \right] \dot{\mathbf{a}} \\\\
/// &= \lim_{\delta \to \mathbf{0}} \frac{1}{\mathbf{\delta}} \mathrm{Exp}(\mathbf{x}) {[\mathbf{\delta}]}\_{\times} \dot{\mathbf{a}}\\;.
/// \end{aligned}
/// $$
///
...
/// $$
pub fn jacobian(rot: &Rotation2, landmark: &Vector2) -> Jacobian {
...
mod tests {
    ...
    fn test_residual() {
        ...
        let transform = Transform::from_se2(&param);
    ...
    fn test_gauss_newton_update() {
        ...
        let true_transform = Transform::from_se2(&true_param);
        let initial_transform = Transform::from_se2(&initial_param);
        ...
        let initial_transform = Transform::from_se2(&initial_param);
        let updated_transform = Transform::from_se2(&updated_param);
    ...
    fn test_icp_3dscan() {
        ...
        let true_transform = Transform::from_se2(&Param::new(0.01, 0.01, -0.02));
        ...
        let noise = Transform::from_se2(&Param::new(0.05, 0.010, 0.010));
